...
int main()
{
    ...
    GPU_Target *win  = GPU_InitRenderer(GPU_RENDERER__OPENGL_3,
            200, 200, GPU_DEFAULT_INIT_FLAGS);
    ...
    /* Mantener conciencia del tiempo para saber cuándo cambiar de sprite */
    ...
    /* La tasa de fotogramas es mejor que sea fija. Usamos el paso del tiempo
    ...
    while (!done) {
        ...
        GPU_BlitTransformX(hero, &rects[position], window, 75, 75, 0, 0, 0, 1,
                1);
        GPU_Flip(win);
        ...
        auto t2 = Clock::now();
        elapsedNano = (double) (std::chrono::duration_cast
                <std::chrono::nanoseconds>(t2 - t1).count());

        if (elapsedNano > 0) {
            double diff = ((1000000000.f / 30.0f) - elapsedNano) / 1000000.f;
        ...
        timeElapsed = (double) (std::chrono::duration_cast
                <std::chrono::nanoseconds>(t3 - t1).count()) / 1000000.f;
    }
